#include "Asset/Serializers/MeshSerializer.h"
...
namespace ge {
#if 0
	static std::pair<filesystem::Path, filesystem::Path> GetSourceAndTargetTexturePathFromDirectory(filesystem::Path& meshDir, const filesystem::Path& meshTargetPath, const filesystem::Path& aiTexturePath) {
		std::string rawStrPath = aiTexturePath.string();
		std::replace(rawStrPath.begin(), rawStrPath.end(), '\\', '/');
		auto texturePath = meshDir / rawStrPath;
		filesystem::Path textureSourcePath = meshDir / rawStrPath;
		filesystem::Path targetTexturePath;
		if (!meshTargetPath.empty()) {
			filesystem::Path textureFilename = textureSourcePath.filename();
	...
	AssetType MeshSourceSerializer::ImportFromSource(const ImportAssetData& asset, const filesystem::Path& source, const filesystem::Path& targetPath)
	{
		...
		filesystem::Path meshDirectory = source.parent_path();
		filesystem::Path targetMeshDirectory = targetPath.parent_path();
		...
		if (scene->HasMeshes()) {
			...
			if (scene->HasMaterials() && spec.loadMaterials) {
				...
				for (uint32_t i = 0; i < materialCount; i++) {
					...
					filesystem::Path matTargetPath = targetPath.parent_path() / (matName + GE_ASSET_EXTENSION);
	...
	void MeshAssetSerializer::SerializeToFile(const filesystem::Path& target, mem::Ref<Asset>& asset, const AssetMetadata& mtd)
	{
	}
